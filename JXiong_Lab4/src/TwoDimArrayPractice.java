...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public int findMinimum(int column) {
       // Initialize minimum to a high value
       int minimum = Integer.MAX_VALUE;

       // Iterate through rows
       for (int row = 0; row < intArray.length; row++) {
           // Get the value in the specified column for the current row
           int currentValue = intArray[row][column];

           // Compare with the current minimum value
           if (currentValue < minimum) {
               minimum = currentValue; // Update minimum if a smaller value is found
           }
   ...
   */
   public int countFound(int value) {
       int count = 0; // Initialize a count variable to keep track of the occurrences of the value

       for (int row = 0; row < intArray.length; row++) {
           for (int column = 0; column < intArray[row].length; column++) {
               if (intArray[row][column] == value) {
                   count++; // Increment the count if the value is found
               }
               animate(row, column, count); // Animate the algorithm with the current count
           }
       }
